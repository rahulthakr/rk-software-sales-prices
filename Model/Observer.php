...
class Thakur_ProductOptions_Model_Observer {  
  ...
  public function getOptionModel(){
    ...
    return new Thakur_ProductOptions_Model_Option();    
  }  
  ...
  public function validate_selected_options($isValid, $productId){
    ...
    foreach ($this->getProductOptions($productId) as $option){
      ...
      if ($option['required'] == 1 && (empty($_POST['pwzrt_option'][$oId]) || (is_array($_POST['pwzrt_option'][$oId]) && $_POST['pwzrt_option'][$oId][0] == ''))){                     
        $isValid = false;
  ...
  public function save_selected_options($cart_item_data, $product_id){ 
    if (isset($_POST['pwzrt_option'])) {   
      $optionValues = (array) $_POST['pwzrt_option'];
      foreach($optionValues as $oId => $value){
        ...
        $cart_item_data['pwzrt_option'][$oId] = $value;        
      }
    }
  ...
  public function display_selected_options_on_checkout($cart_data, $cart_item){
    ...
    $custom_items = array(); 
    if (isset($cart_item['pwzrt_option'])) {
      $custom_items = $this->formatSelectedValues($cart_item['product_id'], $cart_item['pwzrt_option']);
    }
  ...
  public function display_selected_options_with_order_info($item_id, $values, $cart_item_key){
    if (isset($values->legacy_values['pwzrt_option'])){   
      $selectedValues = $this->formatSelectedValues($values->legacy_values['product_id'], $values->legacy_values['pwzrt_option']);      
  ...
  public function add_option_price_on_checkout($cart){
    ...
    foreach (WC()->cart->get_cart() as $key => $citem) {
      ...
      if (!isset($citem['pwzrt_option']) || isset($this->_poPriceAdded[$key]))
        continue;
        
      $selectedValues = $citem['pwzrt_option'];
