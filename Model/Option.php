...
class Thakur_ProductOptions_Model_Option {  
  ...
  public function __construct(){
    ...
    $this->_wpdb = $wpdb;   
    $this->_mainTable = "{$this->_wpdb->base_prefix}pwzrt_product_option";

    include_once(Thakur_PO()->getPluginPath() . 'Model/Option/Value.php');		
    $this->_value = new Thakur_ProductOptions_Model_Option_Value();        		
  }	
  ...
  public function saveOptions($productId, $options){ 
    ...
    foreach($options as $option){
      ...
      $hidePrice = (int) $option['hide_price'];
      $enabled = (int) $option['enabled'];
      $price = isset($option['price']) ? (float) $option['price'] : 0;               
      if ($optionId > 0){
        $this->_wpdb->query("UPDATE {$this->_mainTable} SET title = '{$title}', type = '{$type}', required = {$required}, sort_order = {$sortOrder}, hide_price = {$hidePrice}, price = {$price} , enabled = {$enabled}  WHERE option_id = {$optionId}");                    
      } else {
        $this->_wpdb->query("INSERT INTO {$this->_mainTable} SET product_id = {$productId}, title = '{$title}', type = '{$type}', required = {$required}, sort_order = {$sortOrder}, hide_price = {$hidePrice}, price = {$price}, enabled = {$enabled}");      
