(function ($) {
  ...
  $.widget("Thakur.pwzrtProductOptions", { 
    ...
    _create : function () {
      ...
      this._on({                      
        "click button.pwzrt-delete-option-button": $.proxy(this.deleteOption, this),
        "click button.pwzrt-add-option-button": $.proxy(this.addOption, this),   
        "change select.pwzrt-option-type-select": $.proxy(this.onTypeChange, this),
        "click button.pwzrt-add-option-value-button": $.proxy(this.addRow, this),
        "click button.pwzrt-delete-option-value-button": $.proxy(this.deleteRow, this)                                                                         
      });              
    ...
    addOption : function(e, optionId){
      ...
      var template = wp.template('pwzrt-custom-option-base');
      $('#pwzrt_product_options_container').append(template(data));
      
      if (optionId){
        $('#pwzrt_option_'+optionId+'_type').val(data.type).change();
      }      
    },
    ...
    onTypeChange : function(e){ 
      ...
      if (group == prevGroup){
        ...
        $('#pwzrt_option_'+id+'_type_'+prevGroup).remove();
      }

      var template = wp.template('custom-option-'+group+'-type');
      $('#pwzrt_option_'+id).append(template(data));
    ...
    addRow : function(e, id, rowData){ 
      ...
      $('#pwzrt_select_option_type_row_'+id).append(template(data));              
    },    
